...
router.put(
    ...
    async (req: any, res: any) => {
        try {
            ...
            const { title, description, price, active } = req.body as {
                title?: string;
                description?: string;
                price?: string;
            ...
            if (typeof description !== 'undefined') patch.description = String(description);
            if (typeof price !== 'undefined') patch.price = Number(price);
